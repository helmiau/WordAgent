...
function fixture() {
  ...
  const doc = {
    ...
    get Content() {
      return { Text: this.items.map(p => p.Range.Text).join('') }; 
    },
    get Paragraphs() {
      return { Count: this.items.length, Item: i => this.items[i - 1] }; 
    }
  };
