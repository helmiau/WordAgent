...
### Added
- ask_user 支持单次批量提出 1–4 道必要问题，每题保留 2–4 个选项或自由填写，兼容旧单题记录与断点恢复
- 添加连接测试模型接口和前端按钮
- 添加人在回路功能，添加ask_user工具，添加agent反问用户行为

### Changed
- 将文档强制全文复查改为按需局部检查，默认每个范围最多一轮检查、一轮修正，避免反复润色与读写消耗 token；保留失败、超时后的必要状态恢复
- Microsoft Word 模型连接测试结果移至对应提供商卡片下方，避免提供商删除或重排后结果错位
- 更新py依赖
- 修改README和web文档
- 修改token统计曲线面板
...
- 限制同轮同段落的重复字符格式修正，阻止无进展后的删除重建；文档写操作异常不再自动重放，失败后须重新读取有效目标
- 修复指定文档不存在时误回退到活动文档执行修改的问题
- 优化系统提示词
- 修复pytest测试代码
